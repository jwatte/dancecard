...
export const parseRoomCapacityCSV = (content: string): RoomCapacity[] => {
	...
	for (let i = 1; i < lines.length; i++) {
		...
		if (room && capacityStr) {
			...
			if (isNaN(capacity)) {
				throw new Error(
					`Invalid capacity value "${capacityStr}" for room "${room}". Capacity must be a number.`
				);
			}

			if (capacity <= 0 || capacity >= 1000) {
				throw new Error(
					`Invalid capacity value ${capacity} for room "${room}". Capacity must be greater than 0 and less than 1000.`
				);
			}
